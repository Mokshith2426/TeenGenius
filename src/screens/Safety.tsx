...
import { Shield, ShieldAlert, CheckCircle, AlertTriangle, Send, Sparkles, Eye, EyeOff } from 'lucide-react';
...
export default function Safety() {
  ...
  const [safetyFilter, setSafetyFilter] = useState(() => {
    return localStorage.getItem('TG_SAFETY_FILTER') !== 'false';
  });
  ...
  const handleToggleFilter = () => {
    const nextVal = !safetyFilter;
    setSafetyFilter(nextVal);
    localStorage.setItem('TG_SAFETY_FILTER', String(nextVal));
  };
  ...
  const handleSubmitReport = async (e: React.FormEvent) => {
    ...
    try {
      if (user?.uid === 'sandbox_student_genius_99') {
            ...
            <div className="bg-zinc-50 dark:bg-zinc-950 rounded-2xl p-5 border border-zinc-200/50 dark:border-zinc-850/50 space-y-4">
              <div className="flex items-start justify-between gap-4">
                ...
                <button
                  ...
                  className={`px-4 py-2 text-[10px] font-black uppercase tracking-widest rounded-xl cursor-pointer transition-all ${
                    safetyFilter 
                      ? 'bg-blue-600 hover:bg-blue-500 text-white shadow-md' 
                      : 'bg-zinc-200 dark:bg-zinc-800 hover:bg-zinc-250 text-zinc-700 dark:text-zinc-300'
                  }`}
                >
                  <span className="flex items-center gap-2">
                    {safetyFilter ? <Eye size={13} /> : <EyeOff size={13} />}
                    {safetyFilter ? "Enabled" : "Disabled"}
                  </span>
                </button>
              </div>
