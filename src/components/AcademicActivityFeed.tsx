...
export default function AcademicActivityFeed() {
  ...
  useEffect(() => {
    ...
    const unsubClassrooms = onSnapshot(classroomsQ, (snap) => {
      ...
      const unsubsSubcollections: (() => void)[] = [];
      ...
      visibleClassroomIds.forEach(id => {
        ...
        const unsubNotes = onSnapshot(notesRef, (notesSnap) => {
          notesSnap.docs.forEach(nDoc => {
            ...
            // If it is a file or PDF, categorize as resource
        ...
        unsubsSubcollections.push(unsubNotes);

        // Sub for doubts (which counts as peer questions or upcoming quiz challenges)
        ...
        unsubsSubcollections.push(unsubDoubts);
      });
      ...
      return () => {
        unsubsSubcollections.forEach(unsub => unsub());
      };
    }, () => setLoading(false));
